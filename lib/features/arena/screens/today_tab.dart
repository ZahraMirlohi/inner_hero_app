...
class TodayTab extends StatefulWidget {
  ...
  const TodayTab({super.key, required this.selectedDate});
  ...
  Future<void> _loadData() async {
    ...
    if (user != null && mounted) {
      ...
      for (var task in allTasks) {
        ...
        } else if (_taskFailedStatus[task.id] ?? false) {
          failedTasks.add(task);
        } else if (task.isForDate(widget.selectedDate)) {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (_completedHabits.isNotEmpty ||
                        ...
                        const SizedBox(height: 12),
                        ..._completedHabits.map(
                          (habit) => _buildCompletedHabitItem(habit),
                        ),
                        ..._completedTasks.map(
                          (task) => _buildCompletedTaskItem(task),
                        ),
                        const SizedBox(height: 24),
                      ],

                      if (_failedHabits.isNotEmpty ||
                          _failedTasks.isNotEmpty) ...[
                        Row(
                          children: [
                        ...
                        const SizedBox(height: 12),
                        ..._failedHabits.map(
                          (habit) => _buildFailedHabitItem(habit),
                        ),
                        ..._failedTasks.map(
                          (task) => _buildFailedTaskItem(task),
                        ),
                        const SizedBox(height: 24),
                      ],
